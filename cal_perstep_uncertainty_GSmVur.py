...
def main(args):
    ...
    for idx, example in enumerate(test_data):
        ...
        momentum_velocity, get_answer = 0, False
        step_uncertainty = []
        step_momentum_velocity = []
        step_velocity = []
        step_predicted_uncertainty = []
        last_signal = 0
        ...
        for step_idx in range(args.max_steps):
            try:
                ...
                step_predicted_uncertainty.append(predicted_uncertainty)
                ...
                cur_signal = avg_logp
                cur_velocity = cur_signal - last_signal
                last_signal = cur_signal
                momentum_velocity = args.momentum_rate * momentum_velocity + \
                    (1 - args.momentum_rate) * cur_velocity
                
                current_traj.append(f"Step{step_idx}: {output.text.strip()}")

                # Trigger candidate search if low confidence
                if np.exp(cur_signal) < np.exp(predicted_uncertainty) * args.scaling_rate and step_idx > 0:
                    input_text = build_policy_input(
                ...
                predicted_uncertainty =  np.mean(step_uncertainty) + momentum_velocity
                step_velocity.append(cur_velocity)
                step_momentum_velocity.append(momentum_velocity)
        ...
        all_res.append({
            ...
            'step_predicted_uncertainty': step_predicted_uncertainty,
            'step_velocity': step_velocity,
            'step_momentum_velocity': step_momentum_velocity,
        })
